...
st.caption("XGBoost model trained on 2011-2022 · validated out-of-sample · for research & education only")
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Higher threshold = fewer, higher-conviction trades.")
    ...
    try:
        ...
        with tab2:
            ...
            if "error" in results:
                ...
                fig2 = go.Figure()
                fig2.add_trace(go.Scatter(x=eq.index, y=eq, line=dict(color="lime"),
                                          name="Equity"))
        ...
        with tab3:
            ...
            for feat_name, imp in model.feature_importance(8):
                st.write(f"`{feat_name}` — {imp:.3f}")
...
st.divider()
st.caption("⚠️ This is an experimental research tool, not financial advice. "
           "The model has a thin, validated edge but past performance does not "
           "guarantee future results. Never trade money you can't afford to lose.")